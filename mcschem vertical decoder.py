...
extension='0'
while extension not in ('1', '2'):
    extension=input("Input the number of ticks for extension delay (1-2): ")
    ...
    for x in range(bits):
        ...
        if extension == '2':
            ...
            if not y % 8:
                ...
                schem.setBlock((x*2,y*2-1-2**bits*2, -3), "minecraft:redstone_torch")
                schem.setBlock((x*2,y*2-2**bits*2, -3), "minecraft:red_concrete")
                schem.setBlock((x*2,y*2-2**bits*2, -2), "minecraft:redstone_wall_torch[facing=south]")
                schem.setBlock((x*2,y*2-1-2**bits*2, -2), "minecraft:air")
                schem.setBlock((x*2,y*2-2**bits*2, -1), "minecraft:red_concrete")
                schem.setBlock((x*2,y*2+1-2**bits*2, -2), "minecraft:red_concrete")
...
schem.save("C:/Users/zPippo/curseforge/minecraft/Instances/Redstone/config/worldedit/schematics", str(bits) + "bitVerticalDecoder" + extension + "tExtension", mcschematic.Version.JE_1_18_2)
